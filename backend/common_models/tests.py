...
class AllotmentModelRelationShipTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.subject2 = Subject(
            ...
            course_code="sub3",
            credits=4,
            course_type=Subject.CourseType.ELECTIVE,
            original_lecture_hours=2,
            original_tutorial_hours=0,
            original_practical_hours=4,
            number_of_lecture_batches=2,
            number_of_practical_or_tutorial_batches=6
        )
    ...
    def test_subject_computed_values_sanity_checks(self):
        ...
        self.assertNotEqual(self.subject1.course_code, self.subject2.course_code)
    ...
    def test_subject_allotment_ltp_1_teacher(self):
        self.subject1.allotted_teachers.add(
            self.teacher1,
            through_defaults={
                'allotted_lecture_hours': 3,
...
class ComputedStatusFieldTest(TestCase):
    ...
    def test_allotment_and_assigned_status_PARTIAL(self):
        Allotment.objects.create(
            ...
            teacher=self.teacher1,
            **{
                'allotted_lecture_hours': 2,
                'allotted_tutorial_hours': 4,
                'allotted_practical_hours': 5
            }
        )
        self.assertEqual(self.subject1.allotment_status, AllotmentStatus.PARTIAL)
        self.assertEqual(self.teacher1.assigned_status, AllotmentStatus.PARTIAL)
